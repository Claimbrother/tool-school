...
import AuraDetails from './AuraDetails';
      ...
      <div className={isDetailView ? 'mt-0' : 'mt-8 md:mt-12'}>
        ...
        {!isDetailView && (
          <nav className="flex flex-col gap-4 fade-in">
            <a href="https://smart-mana-it.de/ProfileOverview" className="text-sm text-gray-400 hover:text-white transition-colors">{t('aboutLink')}</a>
            <a href="#contact" className="text-sm text-gray-400 hover:text-white transition-colors">{t('bookingLink')}</a>
          </nav>
        )}
      </div>
      ...
      <div className="relative bg-[#0d0d0d] rounded-[calc(2rem-1px)] p-8 md:p-10 z-10 flex flex-col h-full border border-white/5 shadow-2xl transition-transform duration-300 group-hover:scale-[1.01]">
        ...
        <div className="mt-auto flex flex-col md:flex-row md:items-center justify-between border-t border-white/5 pt-6 gap-6">
          <div className="flex gap-8">
            <div>
              <div className="font-mono text-[9px] text-gray-500 tracking-widest uppercase mb-1.5">{t('formatLabel')}</div>
              <div className="text-white text-xs font-medium">{course.format}</div>
            </div>
            <div>
              <div className="font-mono text-[9px] text-gray-500 tracking-widest uppercase mb-1.5">{t('durationLabel')}</div>
              <div className="text-white text-xs font-medium">{course.duration}</div>
            </div>
            <div>
              <div className="font-mono text-[9px] text-gray-500 tracking-widest uppercase mb-1.5">{t('levelLabel')}</div>
  ...
  return (
    <div className="space-y-8 pb-12 md:mx-4 lg:ml-[6vw] lg:mr-[18vw]">
      <div className="fade-in mb-8">
        <h2 className="text-white text-2xl font-medium tracking-tight mb-2">{t('modulesTitle')}</h2>
        <p className="text-gray-500 text-sm">{t('modulesSub')}</p>
      </div>
      {courses.map(course => (
        <OverviewCourseCard
          key={course.id}
